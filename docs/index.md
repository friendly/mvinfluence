...
![](reference/figures/README-rohwer2-1.png)
...
![](reference/figures/README-rohwer3-1.png)
...
observations, with various methods. These rely on
...
![](reference/figures/README-indexplot-1.png)
...
function
[`heplots::robmlm()`](https://friendly.github.io/heplots/reference/robmlm.html)
uses a simple M-estimator that down-weights cases with large residuals.
...
coefficients in the standard [`lm()`](https://rdrr.io/r/stats/lm.html)
and the robust version. The largest changes are for the coefficients of
the `ss` task, but there is an even greater one for `PPVT` on the `n`
task.
