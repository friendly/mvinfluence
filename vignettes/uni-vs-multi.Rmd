...
Beyond the ill-conditioning diagnosis above, it's often useful to ask a more general question
of a fitted `mlm`: how do the coefficients relate to each other? Two functions answer two
different versions of this question: `car::confidenceEllipse()`^[As of this writing,
`car::confidenceEllipse.mlm()` has a bug when `which.coef` is given as character names for an
`mlm` object: it correctly subsets `vcov()` by name, but the coefficient *center* lookup uses
unnamed indexing into the coefficient matrix and silently returns `NA`, causing a "need finite
'xlim' values" error. Use numeric indices instead, as below, until this is fixed upstream.]
fixes a pair of *coefficients* as the plot axes, while `heplots::coefplot()` fixes a pair of
*responses* and overlays one ellipse per predictor.

We illustrate both with the `schooldata` data from the `heplots` package -- a model with three
response variables (`reading`, `mathematics`, `selfesteem`) and several predictors.
...
response equation -- here, the `occupation` and `visit` coefficients for `reading`, which turn
out to be negatively correlated:

```{r confEllipse-school, fig.width=5, fig.height=4, fig.align='center', echo=-1}
par(mar = c(4, 4, 1, 1)+.1)
