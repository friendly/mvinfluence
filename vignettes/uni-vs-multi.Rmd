...
* `car::confidenceEllipse()`[^car-bug] fixes a pair of *coefficients* as the plot axes.
...
The coefficient table shows the values that `which.coef` indexes into (row-major within each
response column): e.g., `which.coef = c(3, 4)` selects `occupation` and `visit` for `reading`,
the first response column, while `which.coef = c(9, 10)` selects the same two predictors for
`mathematics`, the second column.
